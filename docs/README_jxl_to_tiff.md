...
#### Key feature:
```
 Full ICC profile preservation — when used with `tiff_to_jxl.py`, the exact original 
...
py jxl_to_tiff.py "F:\Photos\photo.jxl" --matrix
...
|------|---------|-------------|----------|
| **Roundtrip** | Has ICC in XMP (default) | djxl auto + attach original ICC | Files from `tiff_to_jxl.py` |
...
**When**: JXL has ICC embedded in XMP CreatorTool (from `tiff_to_jxl.py`)
