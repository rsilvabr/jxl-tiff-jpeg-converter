...
The script has three decode modes, selected automatically based on ICC presence or via flags:

| Mode | Trigger | Description | Best For |
|------|---------|-------------|----------|
| **Roundtrip** | Has ICC in XMP (default) | djxl auto + attach original ICC | Files from `jxl_tiff_encoder.py` |
| **Basic** | No ICC in XMP (default) | djxl auto only, no ICC | Consumer JXLs, web images |
...
**When**: No ICC found in XMP metadata
...
**Use case**: Consumer JXLs from web, cameras, or software that doesn't embed ICC. Assumes sRGB display space.
